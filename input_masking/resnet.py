...
from .sal_layers import *
__all__ = ['ResNet', 'BasicBlock', 'Bottleneck']  # model_registry will add each entrypoint fn to this

class MyDropBlock2d(DropBlock2d):
...
class MyAvgPool2dSame(AvgPool2dSame):
    def forward(self, input):
        if isinstance(input, Tensor):
            return super().forward(input)
        elif isinstance(input, tuple):
...
def create_aa(aa_layer, channels, stride=2, enable=True):
    if not aa_layer or not enable:
        return MyIdentity()
    return aa_layer(stride) if issubclass(aa_layer, MyAvgPool2d) else aa_layer(channels=channels, stride=stride)
...
class BasicBlock(nn.Module):
    ...
    def __init__(
            ...
            reduce_first=1, dilation=1, first_dilation=None, act_layer=MyReLU, norm_layer=MyBatchNorm2d,
        ...
        self.drop_block = drop_block() if drop_block is not None else MyIdentity()
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = MyConv2d(inplanes, first_planes, kernel_size=1, bias=False)
        self.bn1 = norm_layer(first_planes)
        self.act1 = act_layer(inplace=True)
        ...
        self.conv3 = MyConv2d(width, outplanes, kernel_size=1, bias=False)
...
def downsample_conv(
    ...
    norm_layer = norm_layer or MyBatchNorm2d
...
def downsample_avg(
    ...
    if stride == 1 and dilation == 1:
        ...
        avg_pool_fn = AvgPool2dSame if avg_stride == 1 and dilation > 1 else MyAvgPool2d
        pool = avg_pool_fn(2, avg_stride, ceil_mode=True, count_include_pad=False)

    return nn.Sequential(*[
        pool,
        MyConv2d(in_channels, out_channels, 1, stride=1, padding=0, bias=False),
        norm_layer(out_channels)
    ])
...
class ResNet(nn.Module):
    ...
    def __init__(
            ...
            down_kernel_size=1, avg_down=False, act_layer=MyReLU, norm_layer=MyBatchNorm2d, aa_layer=None,
        ...
        if deep_stem:
            ...
            self.conv1 = nn.Sequential(*[
                MyConv2d(in_chans, stem_chs[0], 3, stride=2, padding=1, bias=False),
                norm_layer(stem_chs[0]),
                act_layer(inplace=True),
                MyConv2d(stem_chs[0], stem_chs[1], 3, stride=1, padding=1, bias=False),
                norm_layer(stem_chs[1]),
                act_layer(inplace=True),
                MyConv2d(stem_chs[1], inplanes, 3, stride=1, padding=1, bias=False)])
        else:
            self.conv1 = MyConv2d(in_chans, inplanes, kernel_size=7, stride=2, padding=3, bias=False)
        ...
        if replace_stem_pool:
            self.maxpool = nn.Sequential(*filter(None, [
                MyConv2d(inplanes, inplanes, 3, stride=1 if aa_layer else 2, padding=1, bias=False),
            ...
            if aa_layer is not None:
                if issubclass(aa_layer, nn.AvgPool2d):
                    ...
                    self.maxpool = nn.Sequential(*[
                        MyMaxPool2d(kernel_size=3, stride=1, padding=1),
